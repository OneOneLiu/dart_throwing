...
class RobotBase(object):
    ...
    # added my DH L
    def __parse_dynamics_info__(self):
        numJoints = p.getNumJoints(self.id)
        DynamicsInfo = namedtuple('DynamicsInfo', 
            ['mass','lateral_friction','local_inertia_diagonal','local_inertial_pos','local_intertial_orn','restitution','rolling_friction','spinning_friction','contact_damping','contact_stiffness','body_type', 'collision_margin'])
        self.links = []
        self.controllable_joints = []
        # 19 joints, 19 links
        for i in range(numJoints):
            info = p.getDynamicsInfo(self.id, i)
            info = DynamicsInfo(*[info[j] for j in range(12)]) # use * to show that we want to use the elements of the list
    ...
    def reset_arm(self):
        ...
        for i, joint_id in enumerate(self.arm_controllable_joints):
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, self.arm_rest_poses[i],
    ...
    def move_ee(self, action, control_method):
        ...
        for i, joint_id in enumerate(self.arm_controllable_joints):
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, joint_poses[i],
                                    force=self.joints[joint_id].maxForce, maxVelocity=self.joints[joint_id].maxVelocity/3)
